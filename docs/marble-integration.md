...
```
[Browser /capture/<id>]                   [Server /api/capture/start]
       │                                          │
       │  FormData(meta, photos[4–8])             │  prepareUpload + PUT × N
       └─────────────────────────────────────────►│  → media_asset_id × N
                                                  │  worlds:generate (multi_image)
                                                  │  → operation_id
...
app/api/capture/
  start/route.ts     POST FormData → uploads + generateWorld
