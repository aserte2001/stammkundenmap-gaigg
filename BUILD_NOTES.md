...
### Phase 12 — Marble-Pipeline live + Hobby-Cron-Workaround (2026-05-16) ✅

Erster echter End-to-End-Lauf der Marble-Capture-Pipeline auf Production.

**1. Vercel-Stores provisioniert (User-Aktion):**
- Blob Store `stammkundenmap-gaigg-3dmaps` (öffentlich, für Splat-Mappings JSON)
- Edge Config `stammkundenmap-gaigg-3dmaps` (für `marble:cost:YYYY-MM` Counter)
- Env-Vars in Vercel Project gesetzt: `WORLDLABS_API_KEY`, `CRON_SECRET` (frisch generiertes 64-Char-Hex via PowerShell `RandomNumberGenerator`), `OPENAI_API_KEY`, `MAPBOX_TOKEN`, `NEXT_PUBLIC_MAPBOX_TOKEN`.

**2. Stiller Deploy-Reject diagnostiziert** (gotchas #013):
- Letzter Production-Deploy stand 23h auf altem Commit fest, obwohl GitHub `main` mehrere neue Commits hatte und der Webhook 200 zurückgab.
- Vercel-MCP-Tools (`list_deployments`) bestätigten: keine neuen Builds nach `fbe2d7a`.
- Manueller `vercel deploy --prod` aus dem CLI lieferte die echte Error-Message: `"Hobby accounts are limited to daily cron jobs"`.
- Root Cause: `vercel.json` hatte `"schedule": "*/1 * * * *"` — Hobby-Plan erlaubt nur 1×/Tag.

**3. Lazy-Polling-Architektur (commit `6991433`):**
- Neue `lib/marble/poll.ts:pollCustomerPendingOps(customerId, client?)` als reuseable per-customer-Poll mit dependency-injectable Marble-Client.
- `/api/capture/status` ruft die Funktion lazy auf wenn ein Customer `processing` ist → Browser-Polling treibt Marble forward, kein server-cron nötig.
- `/api/capture/poll` (Cron) bleibt als Tages-Sweep-Fallback für orphaned captures (Tab geschlossen mid-processing). Schedule auf `0 4 * * *` (innerhalb Hobby-Limit).
- Test-Coverage: 9 neue Vitest-Cases in `marble-poll.test.ts` (no-op, in-flight, success, low-res-fallback, error, null-response, network-blip, multi-op-pass, preserved-existing-world).

**4. Erster echter Capture verifiziert** (User vor Ort):
- Customer `c-001` (Bauer Friedrich Pichler), 4 Photos via guided-capture flow.
- Marble Generation: Start 08:14:13 → Ready 08:19:59 = **5:46 min**.
- Output: full_res Splat + 500k + 100k Splat URLs, Thumbnail (.webp), Panorama-PNG.
- World-ID: `3a00446b-4d10-4016-95c6-56a543b05470`.
- Status-Endpoint smoke-test: `curl /api/capture/status?customerId=c-001` → `{"status":"ready", "worlds":[{...}]}`. Lazy-Poll funktioniert.

**5. AzimuthCompass „kalibriert sich…" Hang gefixed** (gotchas #014, commit `1942d0e`):
- 6-Sekunden-Timeout schaltet automatisch auf `ManualAzimuthSelector`.
- Android Chrome nutzt jetzt `deviceorientationabsolute` (true magnetic North) statt relativem `deviceorientation`.
- „Manuell ausrichten"-Button in jedem Compass-State sichtbar (Permission-Prompt, Loading, Aktiv) — User kann jederzeit ausweichen.

**6. Vitest-Race gefixed** (gotchas #015): `fileParallelism: false` in `vitest.config.ts`. Suite-Dauer 5 s → 29 s, dafür stabil.

#### Phase 12 Stats

- **Live URL**: https://stammkundenmap-gaigg.vercel.app
- **Production-Deploy**: `dpl_J2K4zVHh7TYEte7vPoQwmnuVUsGP` (READY) → Nachfolger mit Compass-Fix
- **Tests**: 160/160 grün (151 vorher + 9 neue Marble-Poll)
- **TypeScript**: clean
- **ESLint**: clean (1 pre-existing Warning in `coverage/`, autogenerated)
- **Build**: 12 Routen kompiliert, Turbopack
- **Echte Marble-Welten generiert**: 1 (`c-001`)
- **Marble-Kosten**: ~$0.20 (draft tier, 4 Bilder)
- **gotchas.md**: +3 Einträge (#013, #014, #015)

#### Was noch offen ist

- **Vercel Authentication**: aktuell sind alle Routen public (HTTP 200 ohne Login). Settings → Deployment Protection → "Standard Protection" auf Production einschalten, falls die Capture-/Welt-URLs nicht-öffentlich bleiben sollen. (User-Entscheidung, kostenlos auf Hobby).
- **Weitere Captures**: pipeline ist bereit für `c-002` … `c-015` (alle VIPs). Hard-Cap 100 Welten/Monat ist im Code, aktuell 1/100 verbraucht.
- **Cron-Sweep-Verifikation**: Wird heute um 04:00 UTC erstmals laufen. Falls c-001 nochmal in `processing` zurückgesetzt wird (was nicht passieren sollte), würde der Cron-Sweep es next morning bereinigen.
