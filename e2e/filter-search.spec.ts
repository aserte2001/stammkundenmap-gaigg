...
test.describe("Filter & search", () => {
  test.skip(({ browserName }) => browserName !== "chromium", "Sidebar tests run on desktop chromium");

  test("renders sidebar with filter sections", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/");
    await expect(page.getByLabel("Sidebar mit Kundenliste und Filtern")).toBeVisible({ timeout: 15_000 });
  ...
  test("status filter toggles list count", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/");
    await page.getByRole("button", { name: /Status VIP/ }).first().click();
